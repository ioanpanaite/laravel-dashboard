app.controller('ideaStreamCtrl', function($scope, $state, $timeout, apiService, formatterService,
                                           $stateParams, ideaFactory, contentService, $q, $http ){
    $scope.people = null;

    $scope.hm = hm;

    $scope.currentIdea = ideaFactory;

    contentService.setIdea($stateParams.ideaCode);
    ...
    {
        apiService.get(['file'], {asc: false, idea_code:$stateParams.ideaCode, view:1, sort:'created_at', page:0 }).then(
            function(res){
    ...
    $scope.searchTag = function(term) {
        var tagList = $scope.currentIdea.tags();
    ...
    {
        apiService.get(['quit', $stateParams.ideaCode]).then(
            function(res){
                $state.go('home');
            })
    }
    ...
    $scope.searchPeople = function(term) {
        ...
        return $http.get(apiUrl+'/idea/'+$scope.currentIdea.code()+ '/users', params).then(function (response) {
    ...
    $scope.loadEvents = function(){
        apiService.get(['event'], {idea_code: $stateParams.ideaCode, view: 'short'}).then(
            function(res){
                $scope.events = res;
            }
        )
    ...
    {
        contentService.post($scope.newContent, $scope.shareType, 'idea').then(function(res){
    ...
    $scope.loadStream = function(resetPage)
    {
        ...
        contentService.getIdeaStream(query).then(
            function(data) {
