app.controller('ideaCtrl', function($scope, $state, $stateParams, $timeout, ideaFactory, apiService, $modal){

    $scope.chat = [];
    $scope.chatFromId = 0;

    $scope.ideaCode = $stateParams.ideaCode;
    $scope.currentIdea = ideaFactory;
    ideaFactory.setCurrent($stateParams.ideaCode);
    ...
    {
        apiService.get(['idea', $stateParams.ideaCode, 'nousers']).then(
            function(res){
                $scope.usersToInvite = res;
                var peopleModal = $modal({template: 'partials/modal-idea-invite.html', scope: $scope, animation: 'am-fade-and-slide-top', show: true});
    ...
    $scope.inviteOkClick = function()
    {
        ...
        if(invite.length>0){
            apiService.post(['user', 'invite', $stateParams.ideaCode], {users: invite}).then(
                function(res){
    ...
    {
        if($scope.currentIdea.options() == undefined || $scope.currentIdea.options().features.indexOf('chat') < 0) return;

        var params = {fromid : $scope.chatFromId};
        if(msg) params['message'] = msg;
        apiService.get(['chat', $stateParams.ideaCode], params).then(
            function(res){
                if(res.length > 0)
                {
