app.controller('ideaWikiCtrl', function($scope, $state, $stateParams, $previousState, $sce,$upload,
                                         apiService, growl, ideaFactory, formatterService ){
    ...
    $scope.saveClick = function(){
        ...
        if($scope.editWiki.id > 0){
            ...
            apiService.post(['wiki'], {
                title: $scope.editWiki.title,
                access: $scope.editWiki.access,
                idea_code: $stateParams.ideaCode,
                body: $('#htmleditor').code()
            }).then(
                function(res){
    ...
    {
        var params = {idea_code: $stateParams.ideaCode};
