app.controller('spaceWikiCtrl', function($scope, $state, $stateParams, $previousState, $sce,$upload,
    ...
    $scope.onFileSelect = function($files){
        ...
        for (var i = 0; i < $files.length; i++) {
            ...
            file['id']= i;
            var wiki_id = $scope.wikis.length > 0 ? $scope.wikis[$scope.menuIndex].id : false;
            if (!wiki_id) {
                $scope.newFiles[i].status = 'have'; 
                return false;
            }

            $scope.upload = $upload.upload({
                url: apiUrl + '/file',
                data: {id: fileId, filedate: file.lastModifiedDate, wiki_id: wiki_id},
                file: file})
