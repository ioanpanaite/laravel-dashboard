app.controller('initiativeFilesCtrl', function($scope, $state, $stateParams, $previousState,
                                          apiService, growl, initiativeFactory, formatterService,$popover ){
    ...
    $scope.batchDeleteClick = function()
    {
        angular.forEach($scope.files, function(file){
            ...
            if(file.checked=='1' && (formatterService.isMe(file.user_id) || $scope.currentInitiative.role() >2 )  )
            {
    ...
    $scope.okClick = function()
    {
        ...
        apiService.post(['folder'], {initiative_code: $stateParams.initiativeCode, name: $scope.editFolder, parent_id: parent_id}).then(
            function(res){
                res['childs'] = [];
    ...
    {
        apiService.get(['folder'], {initiative_code: $stateParams.initiativeCode}).then(
            function(data){
                $scope.folders = data;
