 ...
 * Class Meeting
 */
class Meeting extends BaseModel implements Calendarable, Attachable, Commentable, Authorizable
{
     ...
     */
    public function updateField($field, $value, $meetingRole)
    {
        $fieldSet = [
            'title'       => MEETING_ROLE_CREATOR,
            'description' => MEETING_ROLE_CREATOR,
            'state'       => MEETING_ROLE_ASSIGNED,
            'priority'    => MEETING_ROLE_CREATOR
        ];

        if (array_key_exists($field, $fieldSet)) {
            if ($meetingRole >= $fieldSet[$field]) {
                $this->$field = $value;
     ...
     * @return $this
     */
    public static function myMeetings()
    {
        $meetings = static::with('assignedTo')->with('user')->where(
            function ($q) {
...
//            return Auth::User()->inSpace($this->space_id) >= ROLE_MEMBER ;
    ...
    public function syncCalendar($dueDate)
    {
        ...
        if (isset($dueDate) && $dueDate != '') {
            ...
            $calendar = Calendar::where('calendarable_id', $this->id)->where('calendarable_type', 'Meeting')->first();
            if ($calendar) {
    ...
    {
        return $this->belongsToMany('User', 'meeting_users', 'meeting_id', 'user_id')->select('user_id', 'full_name');
    }
