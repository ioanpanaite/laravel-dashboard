...
class UserController extends BaseController
{
    ...
    public function register()
    {
        ...
        $signup_key = Config::get('app.signup_key', false);

        if (!$selfRegistration || $selfRegistration == '') {
            return Responder::json(false)->withMessage('operation_denied')->send();
        }
        
		if(Input::get('user_type') == "organisation"){
		
			$validator = Validator::make(Input::all(), [
				'email'     => 'required|email|unique:users',
				'firm_password'              => 'required|min:6',
				'firm_password_confirmation' => 'required|same:firm_password',
				'firm_terms' => 'required',
        ...
        if ($validator->passes()) {
			...
			if(Input::get('middle_name')){
				$fullname.= Input::get('middle_name');
				$fullname.= ' ';
			}
			if(Input::get('sur_name')){
				$fullname.= Input::get('sur_name');
				$password = Input::get('user_password');
			}
			
			if(Input::get('organisation_name')){
				$fullname.= Input::get('organisation_name');
				$password = Input::get('firm_password');
				$user_type = "organisation";
			}
            
			$act_code = md5(rand(10, 100) . Input::get('email')) . md5(microtime());
			$user = new User;
			$user->email = Input::get('email');
			$user->full_name = ucwords(strtolower($fullname));
			$user->password  = Hash::make($password);
			$user->first_name  = Input::get('first_name');
			$user->sur_name  = Input::get('sur_name');
			...
			$user->activation_expire = date('Y-m-d H:i:s', strtotime("+1 days"));
			$user->admin             = 0;
			$user->create_spaces     = env('DEFAULT_CRETE_SPACES', true);
			...
			if($signup_key != Input::get('signup_key')){
				return Responder::json(false)->withMessage('Only those provided a sign up key can sign up.')->send();
			}
        ...
        // return $this->store($user->id,$act_code);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function store($usr_id='',$act_code='')
    {
        $user = new User;
        $user->email = Input::get('email');
        $user->full_name = ucwords(strtolower(Input::get('full_name')));
        ...
        copy(public_path('assets/avatar/default.jpg'), public_path('assets/avatar/' . $usr_id . '.jpg'));
